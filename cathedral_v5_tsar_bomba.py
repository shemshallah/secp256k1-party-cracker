...
class MoonshineOracle:
    ...
    def _build_bm_strides(self) -> Dict[str, int]:
        ...
        strides = {}
        for sym, order, cent in self._class_map.values() if hasattr(self, '_class_map') else []:
            pass
        # Build directly
        for sym, order, cent, _ in MONSTER_CONJUGACY_CLASSES:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Cathedral v5.0 TSAR BOMBA — secp256k1 ECDLP Solver (qdayproject.com)"
    )
    parser.add_argument("mode", nargs="?", default="test",
                        choices=["test", "demo", "blind", "qday", "known"],
                        help="Execution mode")
